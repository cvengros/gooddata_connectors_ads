  ...
  class TypeConverter
    ...
    class << self
      ...
      def to_database_type(type)
        output = ""
        case type
          when GoodDataConnectorsMetadata::BooleanType
              output = "BOOLEAN"
          when GoodDataConnectorsMetadata::DateType
            if (type.with_time?)
              output = "DATETIME"
            else
              output = "DATE"
            end
          when GoodDataConnectorsMetadata::DecimalType
              output = "DECIMAL(#{type.size},#{type.size_after_comma})"
          when GoodDataConnectorsMetadata::IntegerType
              output = "INTEGER"
          when GoodDataConnectorsMetadata::StringType
